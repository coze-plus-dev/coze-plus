...
import { Table, EmptyState, Spin, Tag, Tooltip, Dropdown, Button, Modal, Toast } from '@coze-arch/coze-design';
...
export const MemberTable = forwardRef<MemberTableRef, MemberTableProps>(({ selectedNode, searchKeyword, onShowDetail }, ref) => {
  ...
  const [resignEmployee, setResignEmployee] = useState<EmployeeData | null>(null);
  
  // 恢复在职弹窗状态
  const [restoreVisible, setRestoreVisible] = useState(false);
  const [restoreEmployee, setRestoreEmployee] = useState<EmployeeData | null>(null);
  
  // 获取员工列表
  const { data: employeeData, loading, run: fetchEmployees } = useRequest(
    async (page = 1, pageSize = 20) => {
      if (!selectedNode || !selectedNode.corpId) return { data: [], total: 0 };
    ...
    },
    {
      ...
      onSuccess: (result) => {
        ...
        setPagination(prev => ({
          ...prev,
          total: typeof result.total === 'string' ? parseInt(result.total, 10) : result.total,
        }));
      },
    }
  );
            ...
            content={
              <div>
                {others.slice(2).map((dept, index) => (
                  <div key={index}>{dept.department_path || dept.department_name}</div>
                ))}
              </div>
            }
            position="top"
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status: any) => {
        ...
        const statusValue = typeof status === 'object' ? status : Number(status);
        if (statusValue === 1) {
          return (
    ...
    {
      title: t(ENTERPRISE_I18N_KEYS.ENTERPRISE_MEMBER_TABLE_COLUMN_DEPARTMENT),
      ...
      render: (_, record: any) => (
        <div className={styles.actionColumn}>
            ...
            render={
              <div className="min-w-[120px] py-[4px]">
                <Dropdown.Menu>
                  ...
                  {record.status === 2 ? (
                    // 离职状态，只显示恢复在职
                    <Dropdown.Item onClick={() => handleAction('restore', record)}>
                      <span>{t(ENTERPRISE_I18N_KEYS.ENTERPRISE_MEMBER_TABLE_ACTION_RESTORE)}</span>
                    </Dropdown.Item>
                  ) : (
                    // 在职状态，显示完整操作菜单
                    <>
                      <Dropdown.Item onClick={() => handleAction('changeDepartment', record)}>
                        <span>{t(ENTERPRISE_I18N_KEYS.ENTERPRISE_MEMBER_TABLE_ACTION_CHANGE_DEPARTMENT)}</span>
                      </Dropdown.Item>
                      <Dropdown.Item onClick={() => handleAction('resignation', record)}>
                        <span>{t(ENTERPRISE_I18N_KEYS.ENTERPRISE_MEMBER_TABLE_ACTION_RESIGNATION)}</span>
                      </Dropdown.Item>
                    </>
                  )}
                </Dropdown.Menu>
          ...
          >
            <Button
              theme="borderless"
              icon={<IconCozMore style={{ color: 'var(--semi-color-text-2)' }} />}
              size="small"
      ...
      <div className={styles.tableContainer}>
        <div className={styles.emptyWrapper}>
          <EmptyState
            ...
            title={t(ENTERPRISE_I18N_KEYS.ENTERPRISE_MEMBER_TABLE_EMPTY_SELECT_NODE_TITLE)}
            description={t(ENTERPRISE_I18N_KEYS.ENTERPRISE_MEMBER_TABLE_EMPTY_SELECT_NODE_DESCRIPTION)}
          />
        </div>
      </div>
    );
    ...
    return (
      <div className={styles.tableContainer}>
        <div className={styles.emptyWrapper}>
          <EmptyState
            icon={<IconCozEmpty className="w-[64px] h-[64px] coz-fg-dim" />}
            title={t(ENTERPRISE_I18N_KEYS.ENTERPRISE_MEMBER_TABLE_EMPTY_NO_EMPLOYEES_TITLE)}
            description={t(ENTERPRISE_I18N_KEYS.ENTERPRISE_MEMBER_TABLE_EMPTY_NO_EMPLOYEES_DESCRIPTION)}
          />
        </div>
      </div>
    );
    ...
    <div className={styles.tableContainer}>
      <div className={styles.tableHeader}>
        <div className={styles.nodeInfo}>
          ...
          <span className={styles.memberCount}>{t(ENTERPRISE_I18N_KEYS.ENTERPRISE_MEMBER_TABLE_TOTAL_MEMBERS)} {pagination.total}</span>
        </div>
      </div>
      ...
      <Table
        tableProps={{
          rowKey: (record: any) => {
            return record.id || String(Math.random());
          },
          dataSource,
          columns,
          pagination: {
      ...
      <Modal
        ...
        width={400}
      >
        <p>{t(ENTERPRISE_I18N_KEYS.ENTERPRISE_RESIGNATION_CONFIRM_CONTENT).replace('{name}', resignEmployee?.name || '')}</p>
